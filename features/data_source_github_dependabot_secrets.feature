Feature: data_source_github_dependabot_secrets
  As a user of the Terraform GitHub Provider
  I want to be able to read the secrets for a GitHub repository's dependabot
  So that I can use them in my Terraform configuration.

  Scenario: Read secrets for a GitHub repository's dependabot
    Given I have a GitHub repository named "test-repo"
    And the repository has a dependabot secret named "MY_SECRET"
    When I define a `data_source_github_dependabot_secrets` resource with the following attributes:
